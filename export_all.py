...
if __name__ == "__main__":
    for i in range(6):
        ...
        params = dvc.api.params_show(f"params{i}.yaml")
        # Seed all RNGs
        seed(params["seed"])
        ...
        os.makedirs(f"outputs/{i}/", exist_ok=True)
        # Save the input and output data for verification purposes later
        np.save(f"outputs/{i}/inp.npy", x.detach().numpy())
        np.save(f"outputs/{i}/out.npy", o.detach().numpy())
        # Export the model graph to QONNX
        export_qonnx(model, (x,), f"outputs/{i}/model.onnx", **params["export"])
